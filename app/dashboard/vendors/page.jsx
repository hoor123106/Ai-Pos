...
export default function Vendors() {
  ...
  const [selectedVendorData, setSelectedVendorData] = useState(null); // Report Popup ke liye
  ...
  // --- Nayi Logic: Vendor Name click karne par report calculate karna ---
  const handleVendorClick = (name) => {
    ...
    // Financial Metrics
    const grossProfit = totalCredit - totalDebit; // Buying (Debit) vs Selling/Returns (Credit)
  ...
  return (
    <div style={{ backgroundColor: "#f9fafb", minHeight: "100vh", padding: "40px", fontFamily: "sans-serif" }}>
        ...
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", backgroundColor: "rgba(0,0,0,0.5)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 2000 }}>
          <div style={{ backgroundColor: "#fff", width: "600px", maxHeight: "90vh", borderRadius: "16px", overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 20px 40px rgba(0,0,0,0.2)" }}>
            ...
            <div style={{ backgroundColor: "#1a5f7a", color: "#fff", padding: "25px", position: "relative" }}>
              ...
              <p style={{ margin: "5px 0 0", opacity: 0.8 }}>Vendor Account: {selectedVendorData.name}</p>
            ...
            <div style={{ padding: "30px", overflowY: "auto" }}>
              ...
              <div style={{ display: "flex", gap: "20px", marginBottom: "25px" }}>
                <div style={{ flex: 1, background: "#fef2f2", padding: "15px", borderRadius: "10px", border: "1px solid #fecaca" }}>
                  <small style={{ color: "#991b1b", fontWeight: "bold" }}>BUYING (DEBIT)</small>
                ...
                <div style={{ flex: 1, background: "#f0fdf4", padding: "15px", borderRadius: "10px", border: "1px solid #bbf7d0" }}>
                  <small style={{ color: "#166534", fontWeight: "bold" }}>SELLING/RETURNS (CREDIT)</small>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "25px" }}>
                <tbody>
                  ...
                  <tr style={{ borderBottom: "1px solid #eee" }}><td style={{ padding: "10px 0" }}>Net Ledger Status:</td><td align="right" style={{ fontWeight: "bold" }}>{selectedVendorData.grossProfit >= 0 ? "PROFIT" : "LOSS"}</td></tr>
                </tbody>
              </table>

              {/* Mini Balance Sheet */}
              <h4 style={{ borderBottom: "2px solid #1a5f7a", paddingBottom: "8px", color: "#1a5f7a" }}>Mini Balance Sheet</h4>
              <div style={{ background: "#f8f9fa", padding: "15px", borderRadius: "8px", marginBottom: "25px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "5px" }}><span>Vendor Net Assets:</span><span style={{ fontWeight: "bold" }}>{formatValue(selectedVendorData.netBalance)}</span></div>
                <div style={{ display: "flex", justifyContent: "space-between" }}><span>Liabilities & Equity:</span><span style={{ fontWeight: "bold" }}>{formatValue(selectedVendorData.totalDebit)}</span></div>
              </div>
      ...
      {/* Form Drawer (Aapki purani logic) */}
      {showForm && (
        <>
            ...
            <form onSubmit={saveVendor} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
              ...
              <div style={{ display: "flex", gap: "10px" }}>
                <div style={{ flex: 1 }}>
                  ...
                  <input name="debit" type="number" value={form.debit || ""} onChange={handleChange} style={inputStyle} />
                </div>
                <div style={{ flex: 1 }}>
                  <label style={labelStyle}>Credit (Sell/Return)</label>
                  <input name="credit" type="number" value={form.credit || ""} onChange={handleChange} style={inputStyle} />
                </div>
              </div>
